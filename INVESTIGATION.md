...
by a model with large activations. The practical rule while it is
...
- The three llama.cpp changes are in [`patches/llamacpp/`](patches/llamacpp/) and apply with
  ...
  this configuration sets anyway, so it can be skipped; it is included because it fixes the cause
  rather than because the recipe needs it. Each patch's contribution was
...
| [`scripts/audit_figures.py`](scripts/audit_figures.py) | Checks that every distinctive figure quoted in this repository still has a log behind it, across the write-up, the patch headers and working notes. It exists because three load-bearing figures turned out to be cited from memory with no surviving artifact, which re-reading never caught; widening it past this file is what found the third. Its blind spot, whole numbers, is documented in the script and is how a fourth escaped |
