...
set -u
exec 9>~/.inv108.lock; flock -n 9 || { echo "another instance holds the lock"; exit 1; }
D=~/inv108; mkdir -p "$D"
...
add = """        if (!sched->callback_eval) {
            ...
            static const bool bc250_copy = getenv("BC250_COPY_EACH_NODE") != nullptr;
            static unsigned long bc250_copies = 0;
            if (bc250_copy) {
                enum ggml_status ec = ggml_backend_graph_compute_async(split_backend, &split->graph);
                if (ec != GGML_STATUS_SUCCESS) {
                    return ec;
                ...
                // read every result back to the host and throw it away
                static std::vector<char> bc250_buf;
                for (int j = 0; j < split->graph.n_nodes; j++) {
                    struct ggml_tensor * t = split->graph.nodes[j];
                    if (!t->buffer || t->op == GGML_OP_NONE) continue;
                    size_t nb = ggml_nbytes(t);
                    if (nb == 0 || nb > (size_t)64*1024*1024) continue;
                    if (bc250_buf.size() < nb) bc250_buf.resize(nb);
                    ggml_backend_tensor_get(t, bc250_buf.data(), 0, nb);
                    if (++bc250_copies % 50000 == 1) {
                        fprintf(stderr, "BC250COPY copies=%lu\\n", bc250_copies);
                    }
                }
            } else {
            enum ggml_status ec = ggml_backend_graph_compute_async(split_backend, &split->graph);
...
run () { # run <arm> <tag>
  ...
  [ "$arm" = B ] && extra=(BC250_COPY_EACH_NODE=1)
  t0=$(date +%s)
  ...
  t1=$(date +%s)
  log "  $tag arm=$arm: $(grep -aoE 'Final estimate: PPL = [0-9.]+' "$D/${tag}_$arm.log" | grep -oE '[0-9.]+$' || echo FAIL) wall=$((t1-t0))s copies=$(grep -aoE 'BC250COPY copies=[0-9]+' "$D/${tag}_$arm.log" | tail -1 | grep -oE '[0-9]+$' || echo 0)"
}

log "=== A as normal against B with every result read back, eight rounds, alternated"
for r in $(seq 1 8); do
...
log "  rebuild rc=$? set_prec back: $(grep -c 'set_prec(kqv' "$G")  probe removed: $(grep -c BC250COPY "$B")"
...
for a in A B; do
  ...
  lc=$(grep -oE "arm=$a:.*copies=[0-9]+" "$D/log" | grep -oE 'copies=[0-9]+' | cut -d= -f2 | sort -n | tail -1)
  log "  arm $a: $(echo "$v" | grep -c '^10\.0103$') of $(echo "$v" | grep -c .) at 10.0103; median wall ${w}s; copies seen ${lc}; distinct: $(echo "$v" | sort -u | tr '\n' ' ')"
done
